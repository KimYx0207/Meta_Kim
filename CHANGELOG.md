...
- **Claude Code live release evaluation no longer lets unrelated user prompt hooks rewrite its structured evidence.** The evaluator reads the actual installed runtime Agent definition, verifies its declared identity and boundary fields, records the full-file digest, then binds a compact projection through Claude's native `--safe-mode --agents --agent` path. User hooks remain untouched, OAuth/provider selection remains host-owned, and the inline binding is reported separately from a normally loaded custom Agent.
