...
- **The controlled edit-capability probe now states its exact final file bytes.** It requires one `after-<marker>` line and explicitly forbids retaining the `before` marker or adding explanatory text, eliminating a prompt ambiguity that could reject a real successful native Edit invocation and waste a second live probe.
