...
- **Graphify release rebuilds recover two strict upstream serialization edge cases without hiding unsafe output.** A hyperedge wrapped as a single `$text` JSON object is parsed and then subjected to the existing ID/reference checks, while public `https://` labels are no longer mistaken for Windows drive paths. Invalid wrappers, dangling references, real local paths, and incomplete graph identity still block the release.
