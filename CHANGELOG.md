...
- **Graphify release rebuilds recover strict upstream serialization edge cases without hiding unsafe output.** A hyperedge wrapped as a single `$text` JSON object, an exact `{item:[...]}` wrapper around its node list, and strict 0–1 decimal confidence strings are pre-bound and normalized before the existing ID/reference checks. Public `https://` labels are no longer mistaken for Windows drive paths, and safe `~/.meta-kim` display aliases render as `<meta-kim-home>`; invalid wrappers or scores, unsafe alias traversal, dangling references, real local paths, and incomplete graph identity still block the release.
