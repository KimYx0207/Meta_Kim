...
- **Graphify 发布重建现在能严格恢复上游序列化边界，同时不掩盖危险输出。** 单一 `$text` JSON 对象包装的超边，以及节点列表外层精确的 `{item:[...]}` 包装，都会先规范化，再继续执行原有 ID/引用校验；公开 `https://` 标签也不会再被误判为 Windows 盘符路径。非法包装、悬空引用、真实本机路径和不完整图谱身份仍会阻断发布。
