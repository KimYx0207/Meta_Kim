...
- **证据指纹有了官方刷新命令。** `npm run meta:runtime:evidence:refresh-pins` 在源码改动后重算 `repo_projection` 的源文件指纹（并提供 CI 用的 `:check` 变体），被钉住的文件被上游提交触碰后不再需要手写一次性脚本自救。
...
- **Hook 上下文注入改为按内容指纹门控，不再按轮次门控。** 规划连续性在计划与上次注入逐字节一致时降级为一行指针，工作闭合后完全停止注入——重新勾开任意条目即恢复完整投影。记忆召回不再重复注入相同选集；graphify 使用提示从每次搜索命令一次降为每会话一次；子代理规则集按会话+代理去重。新增开关：`META_KIM_GRAPHIFY_CONTEXT=off`、`META_KIM_SUBAGENT_CONTEXT=off`、`META_KIM_DISABLE_RECALL_DEDUPE=1`。Codex 侧子代理治理规则的目标从所有子代理收窄为 `meta-*` 治理代理。
...
- **平台选择不再绕过安装范围。** 只做全局安装的项目显式指定平台时，仅限定挂钩投影，不会额外生成项目里的长期代理、技能或命令。
...
- **全局检查会说明具体该修什么。** 过期挂钩列出缺失项、变更项及各运行时的正确来源；安装检查区分源码更新和安装副本损坏，缺少可信安装依据时仍明确作为诊断失败。
